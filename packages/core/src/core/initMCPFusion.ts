 ...
 * ```
 */
export function initMCPFusion<TContext = void>(): MCPFusionInstance<TContext> {
    return {
        // ── Semantic Verbs ────────────────────────────────
...
const _sandboxFinalizer: FinalizationRegistry<string> | undefined =
    typeof FinalizationRegistry !== 'undefined'
        ? new FinalizationRegistry<string>((label) => {
            console.warn(
                `[mcpfusion] SandboxEngine was garbage-collected without dispose(). ` +
