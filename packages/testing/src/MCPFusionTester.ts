/**
 * MCPFusionTester — In-Memory MVA Lifecycle Emulator
 *
...
import type { MvaMeta } from '@mcpfusion/core';
import type { TesterOptions, MvaTestResult } from './types.js';

// ── MCPFusionTester Class ───────────────────────────────────
 ...
 */
export class MCPFusionTester<TContext> {
    constructor(
 ...
 * ```
 */
export function createMCPFusionTester<TContext>(
    registry: ToolRegistry<TContext>,
    options: TesterOptions<TContext>,
): MCPFusionTester<TContext> {
    return new MCPFusionTester(registry, options);
}
