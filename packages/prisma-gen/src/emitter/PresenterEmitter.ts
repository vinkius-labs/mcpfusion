 ...
 *
 * Security rules:
 * - Fields with `@mcpfusion.hide` are physically absent from the schema
 * - Fields with `kind !== 'scalar' && kind !== 'enum'` are filtered (flat-only MVA)
 * - Fields with `@mcpfusion.describe("...")` get `.describe()` calls
...
export function emitPresenter(model: DMMFModel, annotations: ModelAnnotations): GeneratedFile {
    ...
    // ── Imports ───────────────────────────────────────────
    lines.push(`/**`);
    lines.push(` * ${name}Presenter — Generated by @mcpfusion/prisma-gen`);
    lines.push(` *`);
    lines.push(` * Egress Firewall: fields marked @mcpfusion.hide are physically absent.`);
    lines.push(` * This file is auto-generated. Manual edits will be overwritten.`);
    lines.push(` *`);
    lines.push(` * @generated`);
    lines.push(` */`);
    lines.push(`import { z } from 'zod';`);
    lines.push(`import { createPresenter } from '@mcpfusion/core';`);
    lines.push(``);
    ...
    for (const field of scalarFields) {
        ...
        // @mcpfusion.hide — physically exclude from Response
        if (ann?.hidden) continue;

        let zodExpr = prismaTypeToZod(field);

        // @mcpfusion.describe — inject .describe() for LLM semantics
        if (ann?.description) {
