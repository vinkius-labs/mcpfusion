...
description: "llms.txt and SKILL.md are open standards any framework can adopt. Most use them for documentation. MCP Fusion treats SKILL.md as an executable specification — every Fluent API method, every Presenter rule, every anti-pattern — so your AI agent writes correct code on the first pass."
tags:
  - skill-md
  - llms-txt
  - mcp
...
- [How MCP Fusion Uses SKILL.md Differently](#how-mcpfusion-uses-skill-md)
...
MCP Fusion doesn't use SKILL.md as a getting-started guide. It uses it as a **typed behavioral contract** — a document that declares:
...
That last one is the critical piece. Most SKILL.md files tell the agent what to do. MCP Fusion also tells the agent what **not** to do:
...
| Aspect | Typical SKILL.md (guide) | MCP Fusion SKILL.md (executable contract) |
|---|---|---|
...
In MCP Fusion, security primitives are part of the SKILL.md contract:
...
```
fusion.lock — SHA-256 hash per tool contract
```
...
If you want **an AI agent that writes production-grade, security-compliant MCP servers on the first prompt**: MCP Fusion ships the only SKILL.md we've found that functions as an executable architectural contract.

Other frameworks will likely adopt this approach. But today, MCP Fusion is the one that's shipping it.

---

**[Read the SKILL.md specification →](https://agentskills.io)**

**[Get started with MCP Fusion →](https://mcpfusion.vinkius.com/quickstart)**
