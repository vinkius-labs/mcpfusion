...
<div style="font-size:16px;color:rgba(255,255,255,0.7);margin-top:12px;line-height:1.6;font-style:italic;font-family:Inter,sans-serif">"Create a Cloudflare Worker that deploys my MCP Fusion registry to 300+ edge locations using cloudflareWorkersAdapter with D1 and KV bindings."</div>
<!-- Action Bar -->
...
<div style="font-size:14px;color:rgba(255,255,255,0.4);margin-top:16px;max-width:540px;line-height:1.7;font-family:Inter,sans-serif">Deploy your MCP mcpfusion server to Cloudflare Workers. No transport hacks, no session workarounds. Your existing tools, middleware, Presenters, and governance lockfile run at the edge — unchanged.</div>
</div>
...
Peer dependencies: `@mcpfusion/core` (^2.0.0), `@modelcontextprotocol/sdk` (^1.12.0).
...
| `serverName` | `string` | `'fusion-edge'` | MCP server name (visible in capabilities negotiation) |
...
- **MCPFusionClient** — the built-in tRPC-style client
