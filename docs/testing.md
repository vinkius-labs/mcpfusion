...
<div style="font-size:16px;color:rgba(255,255,255,0.7);margin-top:12px;line-height:1.6;font-style:italic;font-family:Inter,sans-serif">"Write Vitest tests that assert PII stripping, middleware access control, system rules presence, and agent limit truncation using MCPFusionTester."</div>
<!-- Action Bar -->
...
Works with Vitest, Jest, Mocha, or `node:test`. The tester returns plain objects — your runner, your choice. Ideal for validating hand-written tools and auto-generated tools from [@mcpfusion/openapi-gen](/openapi-gen) or [@mcpfusion/prisma-gen](/prisma-gen).
...
`createMCPFusionTester` wraps your real `ToolRegistry` and calls `routeCall()` — the same function production uses. No pipeline reimplementation, no mock transport.
...
`ResponseBuilder.build()` attaches MVA metadata via `Symbol.for('MCP Fusion.mva-meta')`. Symbols are invisible to `JSON.stringify`, so the MCP transport never sees them — but `MCPFusionTester` reads them in RAM:

```typescript
// MCP transport sees:
{ "content": [{ "type": "text", "text": "<data>...</data>" }] }

// MCPFusionTester reads (Symbol key):
...
| Guide | Description |
|---|---|
| [Quick Start](/testing/quickstart) | Build your first MCPFusionTester in 5 minutes |
