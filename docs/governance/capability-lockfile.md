...
description: "Generate, verify, and integrate fusion.lock into CI/CD. A deterministic, git-diffable snapshot of the behavioral surface."
---
...
<div style="font-size:16px;color:rgba(255,255,255,0.7);margin-top:12px;line-height:1.6;font-style:italic;font-family:Inter,sans-serif">"Generate a fusion.lock for my server, add it to git, and create a GitHub Actions step that runs fusion lock --check on every PR."</div>
<!-- Action Bar -->
...
```text
Developer builds server → fusion lock → fusion.lock → git commit
...
import {
  generateLockfile,
  writeLockfile,
} from '@mcpfusion/core/introspection';
...
```typescript
import { serializeLockfile } from '@mcpfusion/core/introspection';
...
```typescript
import { readLockfile, checkLockfile } from '@mcpfusion/core/introspection';
...
```typescript
import { parseLockfile } from '@mcpfusion/core/introspection';
