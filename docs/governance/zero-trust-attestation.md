...
```text
[fusion] Zero-Trust attestation failed:
...
```typescript
import type { AttestationSigner } from '@mcpfusion/core/introspection';
...
After attestation, the server can expose a `mcpfusionTrust` capability in the MCP `server.capabilities` object. This allows MCP clients to verify the server's behavioral identity before trusting tool responses:

```typescript
import { buildTrustCapability } from '@mcpfusion/core/introspection';
...
The resulting `MCPFusionTrustCapability` object:
...
```typescript
import { AttestationError } from '@mcpfusion/core/introspection';
...
```typescript
import { verifyAttestation } from '@mcpfusion/core/introspection';
        ...
        run: |
          DIGEST=$(node -e "
            import('./src/server.ts').then(mod => {
              const { compileContracts, computeServerDigest } = require('@mcpfusion/core/introspection');
...
deploy:
  ...
  steps:
    ...
    - name: Verify attestation
      run: |
        node -e "
          import { compileContracts, computeServerDigest } from '@mcpfusion/core/introspection';
