...
The `inputFirewall()` function returns a standard MCP Fusion middleware. It runs **before** your handler, evaluating all arguments through a [JudgeChain](/security/judge-chain):
...
```typescript
import { inputFirewall } from '@mcpfusion/core';
