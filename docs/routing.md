...
<div style="font-size:14px;color:rgba(255,255,255,0.4);margin-top:16px;max-width:540px;line-height:1.7;font-family:Inter,sans-serif">Instead of overwhelming the AI with hundreds of endpoints, MCP Fusion hierarchically groups tools with discriminators — drastically reducing token consumption.</div>
</div>
...
Pair `autoDiscover()` with `mcpfusion dev` for hot-reload. See the [HMR Dev Server](/cookbook/hmr-dev-server).
