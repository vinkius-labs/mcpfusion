...
MCP Fusion implements the full MCP resource subscription lifecycle: `resources/list`, `resources/read`, `resources/subscribe`, and `resources/unsubscribe`. Zero overhead when not configured.
...
```typescript
import { ToolRegistry, ResourceRegistry, defineResource } from '@mcpfusion/core';
