...
```typescript
import { initRedactEngine } from '@mcpfusion/core';
...
```typescript
import { compileRedactor } from '@mcpfusion/core';
...
```typescript
import { initMCPFusion, initRedactEngine } from '@mcpfusion/core';

await initRedactEngine();
const server = initMCPFusion({ /* ... */ });
```
