...
head:
  - - meta
    - name: keywords
      content: MVA pattern, Model View Agent, AI agent architecture, MCP server design pattern, Presenter pattern, agent perception layer, MCP Fusion MVA
---

# The MVA Pattern: Model, View, and Agent {#mva-pattern}

MVA (Model-View-Agent) is an architectural pattern for building AI agent backends. It replaces MVC's browser-oriented View with a **Presenter**, a perception layer that shapes what an AI agent sees, knows, and can do next. The Model defines your domain data. The Presenter (View) adds rules, affordances, and security boundaries. The Agent (any LLM) receives structured context instead of raw data.

MCP Fusion is the reference implementation of MVA for the Model Context Protocol.

::: info Prerequisites
Install MCP Fusion before following this guide: `npm install @mcpfusion/core @modelcontextprotocol/sdk`. You can also scaffold a project with [`mcpfusion create`](/quickstart-lightspeed).
:::
...
<div style="font-size:36px;color:#fff;font-weight:700;font-family:Inter,system-ui,sans-serif;letter-spacing:-1.5px;margin-top:12px;line-height:1.1">MVC was not built for agents.<br><span style="color:rgba(255,255,255,0.25)">Every guess is a hallucination.</span></div>
<div style="font-size:14px;color:rgba(255,255,255,0.4);margin-top:16px;max-width:540px;line-height:1.7;font-family:Inter,sans-serif">When a tool returns <code style="font-size:12px;color:rgba(239,68,68,0.6);background:rgba(239,68,68,0.06);padding:1px 5px;border-radius:3px">{ amount_cents: 45000 }</code>, the agent has to guess: is that cents or dollars? Should it offer a payment action? What visualization fits?</div>
</div>
...
<div style="font-size:12px;color:rgba(245,158,11,0.35);line-height:1.7;font-family:Inter,sans-serif">No affordances. The agent invents tool names and skips required workflows.</div>
</div>
...
## How the MVA Flow Works {#solution}
...
<div style="font-size:12px;color:rgba(255,255,255,0.4);margin-top:4px;line-height:1.6;font-family:Inter,sans-serif">Defined with <code style="font-size:10px;color:rgba(129,140,248,0.6);background:rgba(129,140,248,0.06);padding:1px 5px;border-radius:3px">defineModel()</code>. Declares field types, defaults, fillable profiles, hidden fields, and guarded fields. One source of truth for your domain entity.</div>
</div>
</div>
...
<div style="font-size:12px;color:rgba(255,255,255,0.4);margin-top:4px;line-height:1.6;font-family:Inter,sans-serif">Shapes perception with rules, UI blocks, affordances, and Zod validation. Defined at the domain level, not the tool level. You create <code style="font-size:10px;color:rgba(34,211,238,0.6);background:rgba(34,211,238,0.06);padding:1px 5px;border-radius:3px">InvoicePresenter</code> once, then reuse it in every tool that returns an invoice.</div>
</div>
</div>
...
<div style="font-size:12px;color:rgba(255,255,255,0.4);margin-top:4px;line-height:1.6;font-family:Inter,sans-serif">Receives structured context, not raw data. The agent follows explicit affordances instead of guessing what to do next. Works with Claude, GPT, Gemini, or any MCP-compatible client.</div>
</div>
</div>

</div>

## The Model Layer {#schema-validation}

The Model defines the shape of your domain data. Fields not declared in `m.casts()` or excluded via `m.hidden()` are invisible to the agent. `.describe()` annotations are extracted as system rules automatically.
...
Fields not listed in `m.casts()` or excluded by `m.hidden()` never reach the agent. The `.describe()` annotation on each field is extracted as a system rule automatically.

---

## The Presenter (View Layer) {#presenter-responsibilities}

The Presenter is what makes MVA different from MVC. It acts as a perception layer between your data and the AI agent, controlling exactly what the agent sees, how it interprets the data, and what actions it can take next.
...
<div style="font-size:12px;color:rgba(255,255,255,0.35);line-height:1.7;font-family:Inter,sans-serif">Travel with the data, not in a global prompt. They adapt to user role, tenant, and locale at runtime.</div>
</div>
...
When the agent is working with users or orders, invoice rules are not loaded. This is what MCP Fusion calls **Context Tree-Shaking**: only the rules relevant to the current data are sent to the LLM.
...
```typescript
import { createPresenter, suggest } from '@mcpfusion/core';
...
Presenters compose just like React components. Embed a `ClientPresenter` inside an `InvoicePresenter`, and its rules and UI blocks merge automatically. Define once, reuse across `InvoicePresenter`, `OrderPresenter`, and `ContractPresenter`.
...
`ClientPresenter` rules and UI blocks merge into the parent automatically. Define a presenter once and reuse it everywhere that entity appears.

## Connecting the MVA Pipeline {#pipeline}

Here is a complete MVA tool. The handler (Model) produces raw data. The Presenter (View) shapes perception. The LLM (Agent) acts on structured context.
...
The handler queries your database and returns raw data. `.returns(InvoicePresenter)` wraps that data in rules, affordances, and validation before the agent sees it. The agent never touches unstructured output.
