...
const BASE_URL = 'https://mcpfusion.vinkius.com';
...
const pages: Record<string, PageSEO> = {
  ...
  // ═══════════════════════════════════════════════════════
  'index.md': {
    title: 'MCP Fusion — The TypeScript Framework for MCP Servers',
  ...
  'introduction.md': {
    ...
    description: 'Learn how to build a secure MCP server in TypeScript with MCP Fusion. Covers Presenters, PII redaction, auth middleware, and deployment. Step-by-step with code examples.',
    faqs: [
      { q: 'What is the best framework for building MCP servers in TypeScript?', a: 'MCP Fusion is a TypeScript MCP server framework that adds a structured perception layer on top of the official Model Context Protocol SDK. It provides Presenters (a View layer for AI agents), Zod validation, composable middleware, PII redaction, self-healing errors, and cognitive guardrails. It works with Claude Desktop, Cursor, GPT, and any MCP-compatible client.' },
      { q: 'How do I build a secure MCP server that prevents data leaks to the LLM?', a: 'MCP Fusion prevents data leaks with three mechanisms: Zod .strip() schemas remove undeclared fields before they reach the AI agent, .redactPII() masks sensitive data like SSNs and password hashes, and the Presenter egress firewall guarantees that only declared fields are serialized. New database columns are invisible to the LLM unless explicitly added to the Presenter schema.' },
      { q: 'What is the difference between MCP Fusion and the official MCP SDK?', a: 'The official MCP SDK (@modelcontextprotocol/sdk) provides the protocol transport layer — stdio and HTTP. MCP mcpfusion builds on top of the SDK and adds: MVA architecture with Presenters, action consolidation for grouping thousands of operations, Zod validation, tRPC-style middleware, self-healing errors, PII redaction, cognitive guardrails, State Sync with cache signals, and serverless deployment adapters for Vercel and Cloudflare Workers.' },
      { q: 'Does the MCP Fusion framework work with Claude Desktop and Cursor?', a: 'Yes. MCP Fusion follows the Model Context Protocol standard and works with any MCP-compatible client, including Claude Desktop, Cursor, Windsurf, GPT, Gemini, and custom AI agents built with Vercel AI SDK, LangChain, or LlamaIndex. It supports both stdio transport for local development and HTTP/SSE transport for production deployment.' },
      { q: 'How do I add middleware and authentication to an MCP server?', a: 'MCP Fusion provides a composable middleware pipeline inspired by tRPC and Express. Chain .use() on any tool to add authentication, authorization, rate limiting, or logging. Middleware can derive typed context (e.g., verify a JWT and set ctx.user), and it applies hierarchically to tool groups so a single auth guard can protect hundreds of actions.' },
      { q: 'What is PII redaction in MCP servers and why does it matter?', a: 'PII redaction prevents AI agents from seeing sensitive data like Social Security numbers, password hashes, and medical records. MCP Fusion uses V8-optimized fast-redact to mask PII fields before the response reaches the LLM. This is critical for GDPR, HIPAA, and SOC2 compliance because once data enters an LLM context window, it cannot be retracted.' },
      { q: 'Can I deploy an MCP server built with MCP Fusion to Vercel or Cloudflare Workers?', a: 'Yes. MCP Fusion provides official serverless adapters: @mcpfusion/vercel for Vercel Edge Functions and App Router, and @mcpfusion/cloudflare for Cloudflare Workers with D1, KV, and R2 support. You can also deploy to any Node.js host, AWS Lambda, or self-host with Docker. The framework supports both stdio and HTTP transports.' },
      { q: 'What is the MVA (Model-View-Agent) pattern in MCP Fusion?', a: 'MVA replaces MVC for AI-native applications. Instead of a View that renders HTML, MVA uses a Presenter — a perception layer that structures MCP tool responses for AI agents. The Presenter validates data with Zod schemas, injects domain-specific system rules, suggests next actions, and enforces cognitive guardrails. This prevents LLM hallucination and reduces token waste by up to 40%.' },
      { q: 'Is MCP Fusion open source and free to use?', a: 'Yes. MCP Fusion is open source under the Apache 2.0 license. It is free for personal and commercial use. The framework is built and maintained by Vinkius Labs and has an active community of contributors.' },
    ],
  },
  ...
  'quickstart-lightspeed.md': {
    title: 'Create an MCP Server in TypeScript in 4 Steps [Quickstart]',
    description: 'Create and deploy an MCP server in TypeScript with MCP Fusion. Scaffold, develop, test, and ship to production in under 60 seconds. Includes client setup for Claude, Cursor, and Windsurf.',
    faqs: [
      { q: "How do I create an MCP Server for Cursor?", a: "Run `npx mcpfusion create my-server`. It is currently the only MCP framework that automatically pre-configures a `.cursor/mcp.json` file in the scaffold. Literally open your new folder in Cursor, and the connection is instantly live. No manual clicking, no setup wizard config." },
      { q: "Does MCP Fusion work with Claude Code?", a: "Yes, flawlessly. Because Claude Code runs in your terminal, simply start your MCP Fusion HMR dev server and run `claude mcp add my-server npx tsx src/server.ts`. Claude Code instantly perceives your Zod schemas and executes them via the command line with zero latency." },
      { q: "How do I configure Windsurf to use my MCP Server?", a: "Open your `~/.codeium/windsurf/mcp_config.json` and map your MCP mcpfusion server using the standard STDIO transport (`npx tsx src/server.ts`). Windsurf natively understands MCP Fusion's Agentic Affordances and Context Tree-Shaking, executing complex domain tasks perfectly." },
      ...
      { q: "What makes MCP Fusion the best framework for Cline?", a: "Cline is wildly popular for autonomous coding. MCP Fusion's 'Self-Healing Errors' are perfect for Cline. When Cline hallucinates a wrong parameter, MCP Fusion's `.suggestedArgs` instantly tells Cline how to correct itself, preventing endless loops of broken terminal executions." },
      { q: "What is autoDiscover in the MCP Fusion CLI?", a: "It is file-based routing for MCP tools. You NEVER have to manually register tools. Drop a `listUsers.ts` file in your `/tools/users` folder, and `mcpfusion dev` instantly hot-reloads the tool into Cursor, Claude Code, or Windsurf without you even restarting the chat." },
      { q: "How do I safely connect Clayton/Claude to my Postgres Database?", a: "Use `npx mcpfusion create my-api --vector prisma`. This generates a secure Postgres SQL Agent MCP server. Unlike raw SQL access which destroys databases and burns millions of tokens, MCP Fusion wraps your Prisma schema in Zod-stripped Presenters so the AI only accesses safe, confined Egress Firewalls." },
      { q: "Can I scaffold an MCP server hooked directly to my Prisma database?", a: "Yes! The command automatically installs the MCP Fusion Prisma generator. It bridges your `schema.prisma` directly into live MCP tools. Cursor can suddenly read, write, and query your secure local database via Zod-stripped Presenters." },
    ],
  },
  ...
  'quickstart.md': {
    ...
    faqs: [
      { q: 'How long does it take to build an MCP server with MCP Fusion?', a: 'You can have a production-ready MCP server running in under 5 minutes. Define a tool with f.tool(), register it with ToolRegistry, and attach to an MCP server. The framework handles validation, routing, and response formatting automatically. For an even faster path, use npx mcpfusion create which scaffolds a complete project in 30 seconds.' },
  ...
  'enterprise-quickstart.md': {
    ...
    faqs: [
      { q: 'How do I build a production MCP server with MCP Fusion?', a: 'Define typed context with initMCPFusion<AppContext>(), add authentication middleware with f.middleware(), create Presenters with Zod schemas that strip sensitive fields, and attach to the MCP server with contextFactory and createDebugObserver for audit logging.' },
  ...
  'client-integrations.md': {
    title: 'Connect Your MCP Server to Vercel AI SDK, LangChain, and LlamaIndex',
    description: 'Learn how to connect a TypeScript MCP server to Vercel AI SDK, LangChain, and LlamaIndex. Includes code examples for stdio and HTTP transport, Next.js App Router setup, and ReAct agent configuration.',
    faqs: [
      ...
      { q: 'Can LlamaIndex connect to an MCP mcpfusion server?', a: 'Yes. LlamaIndex agents, like the ReAct agent, can use an MCP mcpfusion server to execute external actions. This guarantees that every mutation passes through strictly typed middleware and Presenter logic, preventing LLM-driven data corruption while LlamaIndex handles RAG orchestration.' }
    ]
  },
  ...
  'governance/index.md': {
    ...
    faqs: [
      { q: 'How does MCP Fusion help with SOC2 compliance?', a: 'By providing Immutable Evidence via the Capability Lockfile. Every change to a tool\'s behavioral contract, schema, or system rules requires a git commit to `fusion.lock`. This guarantees full CISO Compliance by ensuring your AI Agent Sandbox has a deterministic, auditable capability surface that never drifts silently.' },
      ...
      { q: 'How does fusion.lock work?', a: 'The lockfile captures every tool surface behavioral contract: action names, parameter schemas, Presenter fields, destructive flags, and middleware chains. CI diffs the lockfile on every PR — reviewers see exactly what changed in the capabilities.' },
    ]
  },
  ...
  'enterprise/observability.md': {
    ...
    faqs: [
      ...
      { q: 'Does MCP Fusion support OpenTelemetry?', a: 'Yes. Pass any MCPFusionTracer-compatible object (including OpenTelemetry Tracer) to attachToServer via the tracing option. Each tool call creates a span with tool name, action, tags, duration, and semantic error classification.' },
  ...
  // ═══════════════════════════════════════════════════════
  'fusion-client.md': {
    title: 'MCPFusionClient — Type-Safe tRPC-style Client',
    description: 'End-to-end type safety from server to client with createMCPFusionClient(). Full autocomplete, compile-time error checking, and type inference.',
    faqs: [
      { q: 'What is MCPFusionClient in MCP Fusion?', a: 'MCPFusionClient provides tRPC-style end-to-end type safety for MCP tools. Created with createMCPFusionClient<TRouter>(transport), it gives full autocomplete and compile-time checking. If you type a wrong action name or wrong argument type, TypeScript catches it before runtime.' },
      { q: 'How does MCPFusionClient type inference work?', a: 'When you define a tool with defineTool() or createTool(), the action names and parameter schemas are captured as TypeScript types. createMCPFusionClient infers these types, providing autocomplete for action names and type-checked arguments — all the way from server to client, zero code generation.' },
      { q: 'What is MCPFusionTransport?', a: 'MCPFusionTransport connects the client to the MCP server. It has one method: callTool(name, args) => Promise<ToolResponse>. Implement it with any transport: direct in-memory calls for testing, HTTP for remote servers, or stdio for local processes.' },
      { q: 'How does MCPFusionClient compare to tRPC?', a: 'Like tRPC, MCPFusionClient infers types end-to-end without code generation. Unlike tRPC, it works over the MCP protocol instead of HTTP. You get the same DX — autocomplete, type checking, refactoring safety — for AI tool calls instead of API routes.' },
      { q: 'Can MCPFusionClient be used for testing?', a: 'Yes. Create a MCPFusionTransport that calls builder.execute() directly in-memory. This gives type-safe, fast unit tests without starting an MCP server. TypeScript catches invalid action names and wrong argument types at compile time.' },
    ],
  },
  ...
  'resource-subscriptions.md': {
    ...
    faqs: [
      { q: 'What are MCP Resource Subscriptions in MCP Fusion?', a: 'Resource Subscriptions enable real-time push notifications for AI agents. Instead of the agent polling for changes, the server pushes notifications/resources/updated when data changes — stock prices, deploy pipeline status, live error logs. The agent subscribes once and receives updates automatically. MCP Fusion implements the full MCP subscription lifecycle: resources/subscribe and resources/unsubscribe.' },
      { q: 'How do I define a subscribable resource in MCP Fusion?', a: 'Use the fluent builder: f.resource("stock_price").uri("stock://prices/{symbol}").subscribable().handle(async (uri, ctx) => {...}). The .subscribable() method marks the resource for push notifications. Alternatively, use defineResource("stock_price", { subscribable: true, handler: ... }). Non-subscribable resources are read-only and never tracked for subscriptions.' },
  ...
  'examples.md': {
    ...
    faqs: [
      { q: 'What examples are available in the MCP Fusion cookbook?', a: 'The cookbook includes 14 patterns: basic CRUD tools, nested group hierarchies, middleware chains, Presenter with system rules, UI blocks (ECharts, Mermaid), cognitive guardrails, self-healing errors, Presenter composition with embed(), streaming progress, MCPFusionClient usage, tag filtering, state sync, TOON encoding, and observability setup.' },
  ...
  'tracing.md': {
    ...
    faqs: [
      { q: 'How does tracing work in MCP Fusion?', a: 'mcpfusion creates one OpenTelemetry-compatible span per tool call with rich semantic attributes: tool name, action, duration, error type, response size, and tags. Uses structural subtyping (MCPFusionTracer/MCPFusionSpan interfaces) — pass trace.getTracer() from @opentelemetry/api directly. Zero overhead when no tracer is set.' },
      ...
      { q: 'Does MCP Fusion tracing require @opentelemetry/api as a dependency?', a: 'No. MCP Fusion uses structural subtyping — MCPFusionTracer and MCPFusionSpan are interfaces that match the real OpenTelemetry types. Any object with startSpan(), setAttribute(), setStatus(), and end() methods works. The real @opentelemetry/api tracer satisfies these interfaces automatically.' },
      { q: 'What span attributes does mcpfusion create?', a: 'Core attributes: mcp.system, mcp.tool, mcp.action, mcp.durationMs, mcp.isError, mcp.error_type. Enterprise metadata: mcp.tags (tool tags for dashboard filtering), mcp.description (tool context), mcp.response_size (billing/quota tracking). Events: mcp.route, mcp.validate, mcp.middleware.' },
  ...
  'api-reference.md': {
    ...
    description: 'Complete API reference for MCP Fusion: builders, registry, presenters, response helpers, middleware, MCPFusionClient, result monad, streaming, and domain models.',
    faqs: [
      { q: 'What are the main exports of MCP Fusion?', a: 'Main exports: createTool(), defineTool(), createPresenter(), ToolRegistry, createMCPFusionClient(), success(), error(), toolError(), toonSuccess(), defineMiddleware(), progress(), succeed(), fail(), ResponseBuilder, ui helpers (ui.echarts, ui.mermaid, ui.summary), cacheSignal(), invalidates(), and createDebugObserver().' },
      { q: 'What TypeScript version is required for MCP Fusion?', a: 'TypeScript >= 5.7 for full type inference support, especially MCPFusionClient and builder APIs. Node.js >= 18 is required as the runtime.' },
  ...
  'testing.md': {
    ...
    description: 'The end of Vibes-Based Testing. MCPFusionTester audits every MVA layer in CI/CD — zero tokens, zero servers, mathematically verifiable. SOC2 compliance for AI pipelines.',
    faqs: [
      { q: 'What is Vibes-Based Testing in AI?', a: 'Vibes-Based Testing is when a developer starts a Node.js server, opens Claude Desktop, types a prompt, waits for the AI to respond, and visually checks the output. This is subjective, non-repeatable, and impossible to put in a CI/CD pipeline. MCP Fusion eliminates this with the MCPFusionTester — deterministic, in-memory pipeline auditing with zero tokens consumed.' },
      { q: 'What is the MCPFusionTester in MCP Fusion?', a: 'MCPFusionTester is the testing framework for MCP Fusion. It runs the real MVA execution pipeline (Zod Validation → Middleware → Handler → Presenter → Egress Firewall) entirely in memory. It returns structured MvaTestResult objects with decomposed data, systemRules, uiBlocks, isError, and rawResponse fields — each assertable independently.' },
      { q: 'How does MCPFusionTester achieve zero token cost?', a: 'MCPFusionTester calls ToolRegistry.routeCall() directly in RAM — the same code path as production but without any MCP transport, server, or LLM API call. Tests execute in ~2ms each with zero API tokens consumed. No OPENAI_API_KEY or ANTHROPIC_API_KEY required in CI.' },
      { q: 'How does MCPFusionTester prove SOC2 compliance?', a: 'MCPFusionTester provides mathematically verifiable assertions: result.data physically lacks passwordHash (SOC2 CC6.1), result.isError is true when role is GUEST (SOC2 CC6.3), result.systemRules contains expected governance directives. These are deterministic — same input produces same output in every CI run.' },
      { q: 'What is the Symbol Backdoor in MCPFusionTester?', a: 'ResponseBuilder.build() attaches structured MVA metadata (data, systemRules, uiBlocks) to the ToolResponse via a global Symbol (MVA_META_SYMBOL). Symbols are ignored by JSON.stringify, so the MCP transport never sees them. MCPFusionTester reads them in memory for structured assertions — no XML parsing, no string regex.' },
      { q: 'Can MCPFusionTester run in GitHub Actions CI/CD?', a: 'Yes. MCPFusionTester has zero external dependencies — no LLM API, no database, no server. Run npx vitest run in any CI/CD pipeline (GitHub Actions, GitLab CI, Azure Pipelines). Tests complete in milliseconds with zero flakiness from API outages or model variance.' },
    ],
  },

  'testing/quickstart.md': {
    title: 'Testing Quick Start — First Test in 5 Minutes',
    description: 'Build your first MCPFusionTester in 5 minutes. Step-by-step from install to first passing SOC2 governance assertion with zero servers and zero tokens.',
    faqs: [
      { q: 'How do I install the MCP Fusion testing package?', a: 'npm install @mcpfusion/testing. Zero runtime dependencies. Only peer dependencies on MCP Fusion and zod. Works with any test runner: Vitest, Jest, Mocha, or Node\'s native node:test.' },
      { q: 'How do I create a MCPFusionTester?', a: 'Use createMCPFusionTester(registry, { contextFactory: () => ({ prisma: mockPrisma, tenantId: "t_42", role: "ADMIN" }) }). The contextFactory produces mock context for every test call. It supports async factories for JWT resolution or database lookups.' },
      { q: 'How do I call a tool action in tests?', a: 'await tester.callAction("db_user", "find_many", { take: 5 }). Returns an MvaTestResult with data, systemRules, uiBlocks, isError, and rawResponse. The MCPFusionTester injects the action discriminator automatically.' },
      ...
      { q: 'Is MCPFusionTester officially supported by Vinkius Labs?', a: 'Yes, MCPFusionTester is an official, first-party testing utility from Vinkius Labs designed specifically to audit the MVA architecture deterministically in CI/CD without relying on LLM vibes.' },
    ],
  },

  'testing/command-line.md': {
    title: 'Command-Line Runner — CLI Reference for MCPFusionTester',
    ...
    faqs: [
      { q: 'How do I run all MCPFusionTester tests?', a: 'npx vitest run. For verbose output: npx vitest run --reporter=verbose. For specific directories: npx vitest run tests/firewall/ (Egress Firewall only) or npx vitest run tests/guards/ (Middleware Guards only).' },
      ...
      { q: 'Do I need special CLI flags to run MCPFusionTester in CI?', a: 'No, MCPFusionTester runs entirely in-memory and consumes zero tokens, so you merely run `npx vitest run` directly in your GitHub Actions or GitLab CI yaml without mocking network requests.' },
    ],
  },

  'testing/fixtures.md': {
    title: 'Fixtures — Test Setup & Context for MCPFusionTester',
    ...
    faqs: [
      { q: 'What is the setup.ts pattern in MCPFusionTester?', a: 'Create tests/setup.ts with a shared MCPFusionTester instance using createMCPFusionTester(). All test files import { tester } from "../setup.js". Centralizes mock data and context configuration.' },
      { q: 'How does context isolation work in MCPFusionTester?', a: 'Context overrides via callAction\'s 4th argument are shallow-merged per call and never persist. call({ role: "GUEST" }) does not affect the next call. The original context object is never mutated.' },
      { q: 'Can I have multiple MCPFusionTester instances?', a: 'Yes. Create adminTester with role ADMIN and guestTester with role GUEST for fundamentally different configurations. Use overrideContext for per-test variations within the same instance.' },
      { q: 'Do fixtures slow down test execution?', a: 'No, because MCPFusionTester skips network IO entirely. The setup and execution remain CPU-bound, consistently executing full MVA pipelines in ~2 milliseconds per test.' },
  ...
  'testing/assertions.md': {
    ...
    description: 'Complete assertion reference for MCPFusionTester: data field absence/presence, systemRules content, uiBlocks verification, isError checks, rawResponse inspection, and composite SOC2 audit patterns.',
    faqs: [
      ...
      { q: 'Does MCPFusionTester validate raw string outputs?', a: 'No. The true power of MCPFusionTester is that it bypasses string regex assertions. Ensure your tests assert the strictly typed MvaTestResult properties instead of raw standard output.' },
    ],
  },

  'testing/test-doubles.md': {
    title: 'Test Doubles — Mocking Context for MCPFusionTester',
    ...
    faqs: [
      { q: 'What gets mocked in MCPFusionTester tests?', a: 'Only the context. MCPFusionTester runs the real pipeline (Zod, middleware, handler, Presenter). You mock the dependencies your handlers call: prisma, HTTP clients, cache layers, and external services.' },
      { q: 'How do I use Vitest spies with MCPFusionTester?', a: 'const findManyFn = vi.fn(async () => [...]). Pass in contextFactory. After calling tester.callAction(), assert: expect(findManyFn).toHaveBeenCalledOnce(). Verify that invalid inputs never reach the database: expect(findManyFn).not.toHaveBeenCalled().' },
      { q: 'How do I test database error handling?', a: 'Create a mock that throws: user: { findMany: async () => { throw new Error("Connection refused") } }. Create a separate MCPFusionTester with this mock. Assert result.isError is true — proving graceful degradation.' },
      { q: 'Should I mock Presenters?', a: 'Never. The primary benefit of MCPFusionTester is auditing what the Presenter does to the raw data returning from your handlers. Mocking the Presenter defeats the purpose of MVA governance testing.' },
  ...
  'testing/egress-firewall.md': {
    ...
    faqs: [
      ...
      { q: 'How does this map to SOC2 compliance?', a: 'SOC2 CC6.1 (Logical Access): passwordHash absent. CC6.7 (Output Controls): only declared schema fields exist. CC7.2 (Monitoring): deterministic, reproducible in CI/CD. All provable via MCPFusionTester assertions.' },
  ...
  'testing/system-rules.md': {
    ...
    faqs: [
      ...
      { q: 'Do system rules survive the MCPFusionTester extraction?', a: 'Yes! They are extracted perfectly via the MVA Symbol backdoor interceptor, allowing you to explicitly count, match, and verify every single string the LLM would have seen.' },
    ],
  },
  ...
  'testing/ui-blocks.md': {
    ...
    faqs: [
      ...
      { q: 'How does MCPFusionTester handle Mermaid blocks?', a: 'The `.mermaid` UI block exposes the raw render string. You can use standard string assertions: `expect(block.text).toContain("graph TD")`.' },
      { q: 'Do I need a real UI to test these blocks?', a: 'No. UI Blocks are rendered server-side as JSON configuration. MCPFusionTester captures this JSON securely without mounting an external DOM or browser.' },
    ],
  },
  ...
  'testing/middleware-guards.md': {
    ...
    faqs: [
      { q: 'How do I test RBAC with MCPFusionTester?', a: 'Use context overrides: callAction("db_user", "find_many", { take: 5 }, { role: "GUEST" }). Assert result.isError is true and result.data contains "Unauthorized". For ADMIN: result.isError should be false.' },
      ...
      { q: 'Does MCPFusionTester trigger middleware chains properly?', a: 'Yes. It invokes the exact same pre-compiled middleware functions as production, capturing errors injected by authentication logic instantly.' },
  ...
  'testing/oom-guard.md': {
    ...
    faqs: [
      ...
      { q: 'Does MCPFusionTester validate inputs with Zod?', a: 'Yes. The `callAction()` method precisely mocks the MCP router passing exactly through the Zod `.parse()` pipeline before allocating any execution cycle to your business logic.' },
  ...
  'testing/error-handling.md': {
    ...
    faqs: [
      { q: 'What is the difference between error and exception in MCPFusionTester?', a: 'isError: true means the pipeline handled the error gracefully and returned a structured MvaTestResult. An unhandled exception would throw — the MCPFusionTester converts most exceptions into isError results.' },
  ...
  'testing/raw-response.md': {
    ...
    faqs: [
      ...
      { q: 'Can I test MCP tool metadata responses?', a: 'Because MCPFusionTester bypasses the JSON-RPC lifecycle, tool list generation is implicitly defined. To test metadata formatting, utilize unit tests directed securely against your Builders.' },
  ...
  'testing/ci-cd.md': {
    ...
    faqs: [
      { q: 'How do I add MCPFusionTester to GitHub Actions?', a: 'Add a workflow with: actions/checkout, actions/setup-node (node 20), npm ci, npx vitest run --reporter=verbose. No API keys needed. No external services. Tests run in ~500ms total.' },
      ...
      { q: 'Do I need mock servers for MCPFusionTester CI jobs?', a: 'No Docker, no mock APIs, and no local stack. The Action Handler receives fully verified inputs without opening ANY network ports.' },
    ],
  },
  ...
  'testing/convention.md': {
    ...
    faqs: [
      { q: 'How should I organize MCPFusionTester test files?', a: 'Four directories by governance concern: tests/firewall/ (Egress assertions), tests/guards/ (Middleware & OOM), tests/rules/ (System Rules), tests/blocks/ (UI Blocks). Plus tests/setup.ts for the shared MCPFusionTester instance.' },
      ...
      { q: 'How do auditors react to MCPFusionTester conventions?', a: 'Auditors appreciate deterministic security architectures. By segregating egress firewall assertions into their own deterministic folder, auditors can verify logical access boundaries with automated precision.' },
    ],
  },
  ...
  'security/index.md': {
    ...
    faqs: [
      ...
      { q: 'Is the MCP Fusion Security Layer fail-open or fail-closed?', a: 'All four security components default to fail-closed. If the LLM judge crashes, times out, or returns an unparseable response, content is blocked — not allowed. Fail-open is an explicit opt-in via failOpen: true for non-critical evaluations only.' },
      { q: 'Does MCP Fusion Security Layer comply with SOC2 and GDPR?', a: 'Yes. The AuditTrail middleware maps to SOC2 controls CC6.1 (logical access), CC6.3 (access monitoring), CC7.2 (system monitoring), and CC7.3 (change monitoring). For GDPR, it addresses Art. 5(1)(c) (data minimization via SHA-256 hashing), Art. 25 (data protection by design), Art. 30 (records of processing), and Art. 32 (security of processing).' },
    ],
  },
  ...
  'security/judge-chain.md': {
    ...
    faqs: [
      { q: 'What is a JudgeChain in MCP Fusion?', a: 'A JudgeChain wraps one or more LLM adapters and orchestrates their evaluation with configurable strategies. It is the foundational primitive shared by both the PromptFirewall and InputFirewall. You provide the LLM adapter (any function that takes a string and returns a string), and the framework handles timeouts, retries, and verdict parsing.' },
  ...
  'security/rate-limiter.md': {
    ...
    faqs: [
      { q: 'How does rate limiting work in MCP Fusion?', a: 'The rateLimit() middleware applies per-key sliding-window throttling. It tracks timestamps instead of simple counts, preventing the boundary burst problem where a fixed window allows 2x requests at the boundary between two periods.' },
  ...
  'security/audit-trail.md': {
    ...
    faqs: [
      { q: 'How does the Audit Trail work in MCP Fusion?', a: 'The auditTrail() middleware wraps every tool call with compliance-ready logging. It captures who (identity), what (tool + action), when (timestamp), with what arguments (SHA-256 hashed for privacy), the outcome (success/error/blocked/rate-limited), and execution duration.' },
  ...
  'dynamic-manifest.md': {
    ...
    faqs: [
      { q: 'What is the Dynamic Manifest in MCP Fusion?', a: 'The Dynamic Manifest is an opt-in MCP Resource (MCP Fusion://manifest.json) that exposes every registered tool, action, and presenter on the server. It uses the native MCP resources/list and resources/read protocol — no custom HTTP endpoints. RBAC filtering ensures each session only sees authorized capabilities.' },
      { q: 'How do I enable the Dynamic Manifest?', a: 'Pass introspection: { enabled: true } to registry.attachToServer(). The server then advertises MCP Fusion://manifest.json via resources/list and serves the manifest on resources/read. Configure a filter callback for RBAC, and set serverName for the manifest header.' },
      ...
      { q: 'What information does the Dynamic Manifest contain?', a: 'The manifest includes: server name, fusion version, MVA architecture label, all registered tools (with tags, descriptions, input schemas), all actions per tool (destructive/readOnly flags, required fields, Presenter references), and all referenced Presenters (schema keys, UI block types, contextual rules flag).' },
      ...
      { q: 'Can I customize the manifest URI?', a: 'Yes. Set introspection: { enabled: true, uri: \'MCP Fusion://custom/v2/manifest.json\' }. The custom URI is used in both resources/list (advertising) and resources/read (serving). The default is MCP Fusion://manifest.json.' },
  ...
  'oauth.md': {
    ...
    faqs: [
      ...
      { q: 'How does token storage work in @mcpfusion/oauth?', a: 'TokenManager stores tokens in ~/.{configDir}/token.json with restricted file permissions (0o600). It checks environment variables first (envVar priority), falls back to file storage. Pending device codes are stored separately with TTL-based expiration, surviving process restarts during the authorization flow.' },
      ...
      { q: 'Is @mcpfusion/oauth provider agnostic?', a: 'Yes. It works with any OAuth 2.0 server that supports the Device Authorization Grant. You configure the authorizationEndpoint and tokenEndpoint for your provider. It has been tested with GitScrum, GitHub, Google, and custom OAuth servers.' },
  ...
  'dx-guide.md': {
    ...
    description: 'Master the mcpfusion developer workflow: HMR dev server, autoDiscover routing, Cursor integration, Claude Code, and debugging tips for maximum productivity.',
    faqs: [
      { q: "What is the fastest way to start an MCP Fusion project?", a: "Run `npx mcpfusion create my-project` to scaffold a fully configured project with TypeScript, ESLint, and the HMR dev server ready to go." },
      ...
      { q: "Does MCP Fusion work well with Cursor and Claude Code?", a: "Yes, perfectly. You can configure Cursor to attach dynamically to the `mcpfusion dev` stdio pipe. When you change your tool code, Cursor instantly sees the updated schemas and handlers without needing a window reload." },
  ...
  'governance/capability-lockfile.md': {
    ...
    faqs: [
      { q: "How do I generate a capability lockfile in MCP Fusion?", a: "Run `npx mcpfusion lock` in your project root. It discovers all tools using autoDiscover, extracts their schemas, and generates an `fusion.lock.json` file. This file should be committed to version control." },
      ...
      { q: "What happens if I forget to update the lockfile?", a: "If you change a tool's TypeScript code but don't run `npx mcpfusion lock`, the CI pipeline will fail when it runs `npx mcpfusion check`. The automated check prevents unrecorded schema mutations from merging into the default branch." },
  ...
  'governance/surface-integrity.md': {
    ...
    faqs: [
      { q: "What is the surface integrity check?", a: "It is a CI/CD process that compares your current code against the committed `fusion.lock.json`. Run `npx mcpfusion check`. If a developer changed a schema without regenerating the lockfile, the check fails, preventing accidental contract mutations." },
      ...
      { q: "What do I do if the integrity check fails locally?", a: "Simply review the code changes you made to the tool definitions. If the changes are intentional (e.g., adding a new parameter), run `npx mcpfusion lock` to record the new correct state. Your subsequent `npx mcpfusion check` will pass." },
      { q: "Does this validate the implementation logic of the handler?", a: "No. Surface integrity strictly verifies the API contract—the exposition layer that the AI agent interacts with (descriptions, schemas, metadata). Inside-the-handler business logic requires standard unit testing with `MCPFusionTester`." },
    ],
  },
  ...
  'governance/contract-diffing.md': {
    ...
    faqs: [
      { q: "How do I see what changed in my MCP tools?", a: "Run `npx mcpfusion diff`. It generates a human-readable report comparing your local tool surface against the main branch lockfile. It highlights new tools, deleted actions, modified parameters, and changes to destructive/idempotent flags." },
  ...
  'governance/blast-radius.md': {
    ...
    faqs: [
      ...
      { q: "How do I calculate the blast radius of an AI tool change?", a: "Run `npx mcpfusion analyze`. It traverses the MVA dependency graph and outputs a risk score and a list of impacted components. This helps QA teams focus testing efforts on the exact areas affected by the change." },
  ...
  'governance/token-economics.md': {
    ...
    faqs: [
      ...
      { q: "How do I optimize prompt size with token economics?", a: "Run `npx mcpfusion tokens`. It identifies expensive tools with bloated descriptions or overly complex schemas. You can optimize them by using TOON descriptions, removing optional fields, or grouping actions to reduce the prompt surface." },
      { q: "Can token economics help me choose an LLM model?", a: "Yes. By analyzing the raw byte-size of your tool routing table, `npx mcpfusion tokens` can recommend whether you need a massive context window (like Claude 3.5 Sonnet) or if your tools are perfectly optimized for a smaller, cheaper local model (like Llama 3)." },
  ...
  'governance/self-healing.md': {
    ...
    faqs: [
      ...
      { q: "How do I test a self-healing error flow locally?", a: "You can write a unit test block using `MCPFusionTester`. You intentionally feed it bad arguments, extract the resulting `toolError` payload, and use an automated asserter to verify that the `.suggestedArgs` properly reflect the expected safe inputs." },
    ],
  },
  ...
  'governance/cli.md': {
    ...
    faqs: [
      { q: "What commands are available in the MCP Fusion Governance CLI?", a: "The CLI provides: `fusion lock` (generate lockfile), `fusion check` (surface integrity), `fusion diff` (contract diffing), `fusion analyze` (blast radius), `fusion attest` (cryptographic verification), and `mcpfusion tokens` (token economics)." },
      { q: "Do I need to install the CLI globally?", a: "No. The highly recommended approach is using `npx @mcpfusion/core` or accessing the `mcpfusion` binary perfectly injected into your project's local `node_modules/.bin` pathway." },
      ...
      { q: "Does the CLI format output for machine parsing?", a: "The `fusion diff` and `mcpfusion tokens` commands both support `--format json`. This allows you to pipe the command outputs dynamically into custom security validation scripts, alerting systems, or chat-ops bots." },
      { q: "Where does the CLI read configuration from?", a: "The CLI automatically looks for the `fusion.config.js` or `fusion.config.ts` file in your directory root. This config informs the CLI about custom path aliases, ignored tools, and active connector plugins." },
    ],
  },
  ...
  'openapi-gen.md': {
    ...
    faqs: [
      { q: 'How do I turn a legacy REST API into an MCP Server?', a: 'Install @mcpfusion/openapi-gen, then run `npx openapi-gen generate -i ./api.yaml`. It instantly transforms thousands of REST endpoints into a fully typed MCP Server. This is the ultimate Legacy API Migration strategy for AI Agents, requiring zero manual coding.' },
  ...
  'prisma-gen.md': {
    ...
    faqs: [
      { q: "How do I generate MCP tools from a Prisma schema?", a: "Install `@mcpfusion/prisma-gen`, add the generator to your `schema.prisma`, and run `npx prisma generate`. This creates CRUD tools (find_many, find_unique, create, update, delete) for each model with Zod validation, Presenters, and typed context. Scaffold a Prisma project with `npx mcpfusion create my-api --vector prisma`." },
  ...
  'n8n-connector.md': {
    ...
    faqs: [
      { q: "How do I expose n8n workflows as MCP tools?", a: "Install `@mcpfusion/n8n`, configure the `N8nConnector` with your n8n instance URL and API key, and call `connector.discover()`. It scans active webhook-triggered workflows and registers them as MCP tools automatically. Scaffold with `npx mcpfusion create my-server --vector n8n`." },
  ...
  'aws-connector.md': {
    ...
    faqs: [
      { q: "How do I expose AWS Lambda functions as MCP tools?", a: "Install `@mcpfusion/aws` and tag your Lambda functions with `mcp:enabled=true`. The `AwsConnector.discover()` scans your AWS account for tagged functions and registers them as MCP tools. Tools inherit IAM permissions — the connector uses the AWS SDK with your configured credentials." },
      ...
      { q: "Are AWS API rate limits a concern?", a: "If an agent enters a tight hallucination loop against an AWS resource, it could trigger AWS throttling (HTTP 429). We highly recommend wrapping your `AwsConnector` tool discovery inside an MCP mcpfusion runtime bulkhead or concurrency limits." },
    ],
  },
  ...
  'cookbook/hmr-dev-server.md': {
    ...
    faqs: [
      { q: "Does MCP Fusion support Hot Module Reload (HMR)?", a: "Yes. Use `npx mcpfusion dev` to start your server. It wraps the Node process and provides native HMR for tools, presenters, and schemas. When you save a file, the MCP server updates handlers instantly without terminating the stdio or SSE connection, meaning your AI agent never loses context." },
      ...
      { q: "Is HMR safe for production servers?", a: "No. The `mcpfusion dev` command injects a lot of DOM and file-watching overhead. In production, you must always run the compiled output using `node dist/server.js` or via a lightweight `tsx` runner for maximum stability and speed." },
  ...
  'cookbook/production-server.md': {
    ...
    faqs: [
      ...
      { q: "Can I host an MCP mcpfusion server on Vercel or Cloudflare Workers?", a: "Absolutely. MCP Fusion provides Edge-compatible adapters enabling you to expose thousands of complex AI tools serverlessly." },
  ...
  'cookbook/runtime-guards.md': {
    ...
    faqs: [
      ...
      { q: "Are runtime guards configurable per environment?", a: "Absolutely. You can inject environment variables into your runtime guards. This allows you to set aggressive concurrency limits in production while relaxing them for local `mcpfusion dev` testing." },
    ],
  },
  ...
  'cookbook/capability-lockfile.md': {
    ...
    faqs: [
      { q: "How do I create a capability lockfile?", a: "Run `npx mcpfusion lock` in the root of your project. This CLI command scans all your registered tools, presenters, and middleware, extracting their exact Zod schemas and structural definitions into an `fusion.lock.json` file." },
      { q: "What should I do with the generated lockfile?", a: "You must commit `fusion.lock.json` to your version control system (e.g., Git). This ensures that every member of the team and your CI/CD pipelines are validating against the exact same intended tool specifications." },
      ...
      { q: "Can a lockfile detect undocumented destructive tools?", a: "Absolutely. When `npx mcpfusion lock` runs, it enforces that any action mutating state must be explicitly annotated. The lockfile freezes these `destructive: true` flags, preventing silent authorization escalation later." },
  ...
  'cookbook/contract-diffing.md': {
    ...
    faqs: [
      { q: "How do I compare two capability lockfiles?", a: "Use `npx mcpfusion diff`. When executed locally or in CI, this command compares your current working tree's tool schemas against the main branch's committed `fusion.lock.json` to produce a detailed structural change report." },
      { q: "What formats can the contract diff output?", a: "By default, `npx mcpfusion diff` provides a color-coded terminal output. However, you can use `--format markdown` to generate a GitHub PR comment, or `--format json` to programmatically pipe the diff into automated security evaluation tools." },
  ...
  'cookbook/blast-radius.md': {
    ...
    faqs: [
      { q: "How do I calculate the blast radius of my MCP tools?", a: "Run `npx mcpfusion analyze`. This statically maps the dependency graph of your server's routing table, pinpointing exactly how many sub-actions, presenters, and rules are impacted when you modify a core shared schema." },
      ...
      { q: "Can the CLI block high-risk deployments?", a: "Yes. You can configure `npx mcpfusion analyze --fail-threshold 80`. If a developer modifies a global auth middleware that impacts every single tool, the blast radius score will exceed 80 and exit with code 1, halting the CI pipeline." },
  ...
  'cookbook/token-economics.md': {
    ...
    faqs: [
      { q: "How can I estimate the LLM token cost of my tools?", a: "Execute `npx mcpfusion tokens`. This built-in governance utility parses your tools' Zod schemas, descriptions, and TOON annotations to provide an estimated prompt overhead in tokens using standard BPE (Byte-Pair Encoding) models." },
      { q: "Can I identify the most expensive tools in my server?", a: "Yes. The `npx mcpfusion tokens` command automatically ranks your tools from most expensive to least expensive based on string density, nested schema depth, and Presenter configuration, letting you pinpoint token hogs immediately." },
  ...
  'cookbook/semantic-probe.md': {
    ...
    faqs: [
      { q: "How do I run a semantic probe on my definitions?", a: "Use `npx mcpfusion probe`. This command securely connects to a judge LLM (using your local API keys) and evaluates the human/AI readability, precision, and contextual clarity of every tool description and parameter schema in your repository." },
  ...
  'cookbook/zero-trust-attestation.md': {
    ...
    faqs: [
      { q: "How do I cryptographically sign an MCP lockfile?", a: "Use `npx mcpfusion attest`. This command digests the current `fusion.lock.json` and uses your enterprise PKI (like AWS KMS) or a local private key to generate an immutable `.sig` attestation artifact." },
      ...
      { q: "How does the MCP mcpfusion server verify the signature at runtime?", a: "During the bootstrap phase, you initialize the `ToolRegistry` with `.requireAttestation(publicKey)`. The framework will verify the `.sig` file against the live memory capabilities graph before opening the SSE or stdio transports." },
  ...
  'cookbook/testing.md': {
    title: 'Testing Recipe — MCPFusionTester Quick Patterns',
    description: 'Common testing patterns with MCPFusionTester: assert PII stripping, verify system rules, check RBAC guards, and validate UI blocks — all in-memory, zero tokens.',
    faqs: [
      { q: "How do I write unit tests for an MCP tool?", a: "Use `MCPFusionTester`. It simulates an exact MCP JSON-RPC call entirely in-memory. You pass the tool name and payload, and it bypasses network transports, instantly returning the structured outcome object for assertions." },
      { q: "How do I test that a Presenter strips PII?", a: "Write a Jest/Vitest check: pass a raw database object containing a password field into the `MCPFusionTester`. Then assert that `expect(result.data.password).toBeUndefined()`. This guarantees your Zod egress schema is correctly pruning dangerous fields." },
      { q: "Can I test Context-Aware System Rules?", a: "Yes. Execute a tool using `MCPFusionTester` with mock context (e.g., role: 'guest'), then assert the resulting `result._systemRules` array. Run it again with `role: 'admin'` and verify the admin-specific instructions successfully injected." },
      { q: "Does MCPFusionTester require an LLM API key?", a: "No! MCPFusionTester strictly tests your server's logic, validation, grouping, and Presenter rendering. It uses zero tokens, costs zero dollars, and executes in milliseconds, making it perfect for CI/CD pipelines." },
  ...
  'jwt.md': {
    ...
    faqs: [
      { q: 'What is the JWT package in MCP Fusion?', a: 'The @mcpfusion/jwt package provides standards-compliant JWT verification for MCP servers. It verifies tokens using jose when installed (supporting RS256, ES256, JWKS auto-discovery) or falls back to native Node.js crypto for HS256 verification. It includes middleware (requireJwt), a tool factory (createJwtAuthTool), and full claims validation (exp, nbf, iss, aud, requiredClaims).' },
  ...
  // ═══════════════════════════════════════════════════════
  'blog/index.md': {
    title: 'MCP Fusion Blog — MCP Server Frameworks, AI Agents & TypeScript',
    ...
    faqs: [
      { q: 'Where can I learn about MCP server frameworks?', a: 'The MCP Fusion blog publishes in-depth technical articles comparing MCP server frameworks (FastMCP, mcp-framework, EasyMCP, the official SDK, and MCP Fusion), along with deep dives into the MVA (Model-View-Agent) pattern, AI agent security, and production deployment strategies.' },
      { q: 'What topics does the MCP Fusion blog cover?', a: 'The blog covers MCP server development, framework comparisons, the Presenter pattern, AI agent security (prompt injection defense, PII redaction), TypeScript best practices for agentic workloads, and real-world case studies of enterprise MCP deployments.' },
      { q: 'Is the MCP Fusion blog useful for developers not using MCP Fusion?', a: 'Yes. Many articles cover general MCP concepts, protocol best practices, and framework comparisons that are valuable to any developer building MCP servers — regardless of their framework choice.' },
    ],
  },

  'blog/posts/introducing-mcpfusion.md': {
    title: 'Introducing MCP Fusion — AI-First DX for the Model Context Protocol',
    description: 'Discover MCP Fusion — a framework that brings the MVA pattern, Presenters, and a world-class developer experience to the Model Context Protocol.',
    faqs: [
      { q: 'What is MCP Fusion?', a: 'MCP Fusion is the AI-First Developer Experience for the Model Context Protocol. It introduces the MVA (Model-View-Agent) architectural pattern alongside a rich toolkit that lets you ship production-ready MCP servers in minutes instead of days.' },
      { q: 'What does MCP Fusion include out of the box?', a: 'MCP Fusion includes Presenters (a deterministic View layer for AI agents), Zod-first tools, a composable middleware pipeline, error self-healing, and a full governance suite with capability lockfiles, contract diffing, and zero-trust attestation.' },
    ],
  },
  ...
  'blog/posts/anatomy-of-an-ai-platform-breach.md': {
    title: 'Anatomy of an AI Platform Breach — How MCP Fusion Defends Every Attack Vector',
    description: 'A rigorous analysis of how MCP Fusion\'s security architecture — InputFirewall, PromptFirewall, AuditTrail, and CapabilityLockfile — would have prevented, detected, or mitigated each vulnerability in a recent Fortune-100 AI platform breach.',
    faqs: [
      { q: 'How does MCP Fusion prevent SQL injection attacks?', a: 'MCP Fusion provides two layers: Zod schema validation eliminates dynamic-key injection structurally (only allowlisted keys reach the handler), and the InputFirewall uses an LLM-as-Judge to semantically detect SQL payloads in values before they reach application code.' },
      { q: 'How does MCP Fusion protect system prompts from tampering?', a: 'System rules in MCP Fusion are defined in source code, not stored in databases. The PromptFirewall evaluates dynamic rules before they reach the LLM, and the CapabilityLockfile tracks SHA-256 digests of all behavioral surfaces in CI.' },
      { q: 'What is the CapabilityLockfile in MCP Fusion?', a: 'fusion.lock is a deterministic, git-diffable lockfile that captures SHA-256 digests of every tool\'s behavioral surface — including system rules, schemas, middleware chains, and entitlements. The CI gate (fusion lock --check) fails if the lockfile is stale, forcing a conscious review of all behavioral changes.' },
    ],
  },
