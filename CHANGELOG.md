...
Monorepo-wide release: all 16 packages move from `5.0.10` to `5.1.0` in lockstep (uniform workspace + npm registry). Only `@mcpfusion/core` carries code changes; the other 15 packages are otherwise untouched — their `@mcpfusion/core` dependency range is already `^5.0.0`, which `5.1.0` satisfies.

### Added — W3C Trace Context correlation + GenAI / OpenInference dual-emit

#### `@mcpfusion/core` — distributed-trace parent correlation + dual-convention tool spans

- **`MCPFusionSpanContext`** (new exported type): a zero-dependency, structurally opaque parent-span handle carrying a W3C `traceId` (32 lowercase hex) / `spanId` (16 lowercase hex) / `flags`, plus optional `remote` / `traceState` / `baggage` / `raw`. The host builds one from an incoming `traceparent` / `tracestate` / `baggage` (HTTP headers, or MCP `params._meta`) and the framework only *forwards* it — it never inspects the value. This keeps `@mcpfusion/core` free of any `@opentelemetry/*` dependency (structural subtyping, as always) while staying interop-compatible: a real OTel `Tracer` maps `traceId` / `spanId` / `raw` onto OTel's `SpanContext` + `Context`.
- **`MCPFusionTracer.startSpan(name, options?, context?)`**: new **optional** third argument `context?: MCPFusionSpanContext`. Omitting it (every pre-existing call site) starts a fresh root span; passing an extracted remote parent makes the MCP tool span parent to the caller's distributed trace instead of minting a new root. Back-compatible — the argument is optional and no existing caller or test changes.
- **W3C helpers** (zero-dep, `globalThis.crypto.randomUUID()`-based; byte-format matches the existing `SwarmGateway` traceparent generator): `W3CFlagSampled`, `newTraceId()`, `newSpanId()`, `generateTraceparent(sampled=true)`, `parseTraceparent()` (strict conformance: version byte `00`, 32/16 lowercase **non-zero** hex ids, two-hex flags — any malformed value → `undefined`, never a fabricated parent), `parseTracestate()` (≤ 512 chars pass-through), `parseBaggage()` (≤ 8192-byte cap), and `extractW3CContext({ traceparent?, tracestate?, baggage? })` → `MCPFusionSpanContext | undefined` (`remote: true`; `undefined` when `traceparent` is absent or malformed).
- **Per-request parent pickup**: `GroupedToolBuilder` reads a conventional, duck-typed key `ctx.mcpTraceContext` (the MCP-session-per-request context, the same pattern as `ctx.handoffTraceparent`) and threads it into the tool span; `ToolRegistry` does the same for the unknown-tool span so even 404 routing stays correlatable. A new `readMcpTraceContext(ctx)` helper performs the single property read inside a `try/catch` so the missing-context guard proxy that `attachToServer()` substitutes when no `contextFactory` is provided is tolerated — absence of a trace context is "the span is a fresh root", never an error.
- **G2 dual-convention emission**: every tool span now also carries `openinference.span.kind = 'TOOL'`, `gen_ai.operation.name = 'tools/call'`, and `tool.name`, alongside the native `mcp.*` attributes, so the same span is portable across Datadog, Arize and Phoenix without the backend needing to understand `mcp.*`. The unknown-tool (404) span emitted by `ToolRegistry.routeCall` carries the same three tags — otherwise a failed routing attempt would be invisible to any backend query filtered by `openinference.span.kind = 'TOOL'`. No token/cost attributes are emitted at the tool-execution boundary — those belong on the agent-side LLM span, and are intentionally not fabricated here.
- Added `packages/core/tests/observability/Tracing.test.ts` blocks pinning: G2 dual-emit attributes on the tool span (and parity on the unknown-tool span); G1 W3C helper round-trips + malformed-input rejection (bad version / all-zero ids / wrong length / uppercase / missing → `undefined`); direct `readMcpTraceContext` coverage including the missing-context guard proxy that throws on any property get; and parent-context propagation (a `ctx.mcpTraceContext` from `extractW3CContext` threads into both the tool span and the unknown-tool registry span; absence of one leaves the span a fresh root).
