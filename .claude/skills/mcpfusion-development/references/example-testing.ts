 ...
 *
 * Demonstrates @mcpfusion/testing for in-memory MVA lifecycle emulation.
 * The MCPFusionTester runs the REAL pipeline (Zod → Middleware → Handler → Presenter → Egress)
 * in RAM — zero tokens consumed, zero servers, deterministic.
 *
 * Key patterns shown:
 *   - createMCPFusionTester() — in-memory test harness
...
import { initMCPFusion, defineModel, definePresenter } from '@mcpfusion/core';
